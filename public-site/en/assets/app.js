(function () {
  ...
  function favButtonHTML(id, label) {
    ...
    return `<button type="button" class="fav-btn${active ? " is-active" : ""}" data-fav-toggle="${escapeHtml(id)}" aria-pressed="${active}" aria-label="${escapeHtml(label || "Favorite")}">${heartIconSVG}</button>`;
  }
  ...
  function bindAffiliate(el, product) {
    ...
    el.addEventListener("click", (ev) => {
      ...
      if (!url) {
        ev.preventDefault();
        alert("This deal doesn't have a destination link set up yet.");
      }
    });
  }
  ...
  function cardHTML(p) {
    ...
    return `
      <article class="product-card" data-category="${escapeHtml(p.category)}" data-id="${escapeHtml(p.id)}">
        ${favButtonHTML(p.id, "Favorite this deal")}
        ...
        <div class="product-body">
          ...
          <div class="product-bottom">
            ...
            <a class="btn primary" data-cta="${escapeHtml(p.id)}" href="${escapeHtml(affiliateUrl(p) || "#")}">View deal</a>
          </div>
        </div>
      </article>`;
  }
  ...
  function renderGrid(list) {
    ...
    if (!grid) return;
    if (!list.length) {
      grid.innerHTML = `<div class="empty">No offers in this selection. Try another filter or search term.</div>`;
      return;
    }
  ...
  function storeInitials(store) {
    return String(store || "Store").slice(0, 2).toUpperCase();
  }

  function storeChipHTML(p) {
    return `<span class="store-chip store-chip-${escapeHtml(p.storeKey || "")}">${escapeHtml(storeInitials(p.store))}</span> ${escapeHtml(p.store || "Store")}`;
  }
  ...
  function pickCompareSet(list, n) {
    ...
    if (cheapest) { picks.push({ item: cheapest, label: "Great price" }); used.add(cheapest.id); }
    ...
    for (const p of pool) {
      ...
      picks.push({ item: p, label: "Also worth it" });
      used.add(p.id);
    }
  ...
  function compareCardHTML(pick, rank, isCheapest) {
    ...
    return `
      <article class="compare-card${rankClass}" data-id="${escapeHtml(p.id)}">
        ...
        ${isCheapest ? `<span class="compare-best">✓ Lowest price in category</span>` : ""}
        <a class="btn ${rank === 1 ? "primary" : "outline"}" data-cta="${escapeHtml(p.id)}" href="${escapeHtml(affiliateUrl(p) || "#")}">View deal</a>
      </article>`;
  }
  ...
  async function renderComparisonsSection() {
    ...
    try {
      ...
      if (!Array.isArray(items) || !items.length) {
        grid.innerHTML = `<div class="empty">We haven't published any comparisons yet. Check back soon.</div>`;
        return;
      }
      const sorted = [...items].sort((a, b) => new Date(b.updatedAt) - new Date(a.updatedAt)).slice(0, 6);
      grid.innerHTML = sorted.map((item) => `
        <article class="product-card">
          ${favButtonHTML(item.slug, "Favorite this comparison")}
      ...
      grid.innerHTML = `<div class="empty">Couldn't load comparisons right now.</div>`;
    }
  }
  ...
  function renderCompareGrid() {
    ...
    if (!picks.length) {
      grid.innerHTML = `<div class="empty">We don't have a published comparison yet.</div>`;
      return;
    }
    ...
    if (title) title.textContent = `Best offers for "${picks[0].item.title}"`;
    const subtitle = document.getElementById("compareSubtitle");
    if (subtitle) subtitle.textContent = "Compare prices across the top stores and pick the best option for you.";
  }
  ...
  function renderDealPage() {
    ...
    if (!p) {
      root.innerHTML = `<div class="empty"><h1>Offer not found</h1><p>This listing has changed. See the active deals on the home page.</p><p><a class="btn primary" href="index.html">Back to deals</a></p></div>`;
      document.title = "Offer not found — Achado Agora";
      return;
    }
    ...
    root.innerHTML = `
      <section class="deal">
        ...
        <aside class="deal-panel">
          ...
          <p class="legal-meta">Reference price checked on ${escapeHtml(p.updatedAt || cfg.priceCheckedOn)}. The final price is set by the store at checkout.</p>
          <p><a class="btn primary full" data-cta="${escapeHtml(p.id)}" href="${escapeHtml(affiliateUrl(p) || "#")}">View deal on ${escapeHtml(p.store)}</a></p>
        </aside>
      </section>
      <section class="ad-slot" aria-label="Advertisement">
        <span>Advertisement</span>
        <small>Space for display network.</small>
      </section>
    ...
    <section class="related">
        <div class="section-head"><h2>Other deals</h2></div>
        <div class="grid" id="productGrid"></div>
      </section>
      <div class="sticky-cta" aria-label="Shortcut to offer">
        <div>
          <span>Preco ref.</span>
          <strong>${formatBRL(p.price)}</strong>
        </div>
        <a class="btn primary" data-sticky-cta="${escapeHtml(p.id)}" href="${escapeHtml(affiliateUrl(p) || "#")}">Open deal</a>
      </div>`;
  ...
  const searchCopy = {
    ...
    "view": "View deal",
    "price": "View price",
    "checked": "Price checked on",
    "cachedCard": "Saved deal",
    "results": "Deals found for",
    "ready": "Find your next deal",
    "start": "Enter the product you want and click Search.",
    "searching": "Searching for",
    "busy": "Checking available stores. This may take up to 90 seconds.",
    "empty": "No matching deals were found with these filters. Add a brand or model, or adjust the price range.",
    "partialEmpty": "Some stores could not be checked, and no offers were found in the stores that responded. Try again shortly.",
    "errorTitle": "Search could not be completed",
    "error": "Search is temporarily unavailable. Please try again shortly.",
    "limited": "Too many searches in a short time. Please wait before trying again.",
    "timeout": "The search took longer than expected. Please try again shortly.",
    "noLive": "Current prices could not be checked. These deals are from an earlier search.",
    "saved": "Some deals are from earlier searches. Confirm the price and availability at the store.",
    "partial": "Some stores did not respond. These are the deals available from this search.",
    "finalPrice": "Prices and shipping may change. Confirm the total and availability at the store.",
    "invalid": "Enter at least 2 characters to search.",
    "invalidPrice": "Enter valid prices, with the minimum no higher than the maximum.",
    "waiting": "Click Search to check this product.",
    "savedFilters": "Preferences saved on this device.",
    "saveError": "Your browser could not save preferences. Your filters still apply to this search.",
    "restored": "Default preferences restored.",
    "activeSaved": "Your saved preferences are active.",
    "changed": "Filters updated. Applying them to the search.",
    "priority": "Priority",
    ...
    "preparing": "No deals are available for this search yet. Please try again later."
  };
      ...
      if (profile && profile.name) {
        body.innerHTML = `
          <div class="modal-profile-meta">
            <div>Hi, <strong>${escapeHtml(profile.name)}</strong></div>
            <a href="favoritos.html">View favorites</a>
          </div>
          <p>Your profile is saved in this browser to remember your favorites. It's not a password account and doesn't sync between devices.</p>
          <div class="modal-actions">
            <button type="button" class="btn-line" id="profileLogoutBtn">Log out</button>
            <button type="button" class="btn primary" id="profileCloseBtn">Close</button>
          </div>`;
        ...
        body.innerHTML = `
          ...
          <p>We only save a name in this browser so you can recognize your favorites. No password, no email, no syncing between devices.</p>
          <form id="profileForm">
            <input type="text" name="name" placeholder="What should we call you?" maxlength="40" required autocomplete="off" />
            <div class="modal-actions">
              <button type="button" class="btn-line" id="profileCancelBtn">Cancel</button>
