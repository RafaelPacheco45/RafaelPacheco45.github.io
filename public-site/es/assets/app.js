(function () {
  ...
  function loadDisplayAds() {
    ...
    if (!validAdsenseClient(client)) {
      slots.forEach((slot) => {
        ...
        slot.innerHTML = "<span>Publicidad</span><small>Cliente de AdSense inválido. Usa ca-pub-0000000000000000.</small>";
      });
      return;
    }
  ...
  function searchProductCardHTML(item, label) {
    ...
    return `
      <article class="product-card" data-search-product="${escapeHtml(item.id)}">
        ${favButtonHTML(item.id, "Marcar oferta como favorita")}
        ...
        <div class="product-body">
          ...
          <div class="price-row">
            ${item.price ? `<strong class="price">${formatBRL(item.price)}</strong>` : `<strong class="price">Ver precio</strong>`}
  ...
  function bindShoppingResultLinks(root, items) {
    ...
    root.querySelectorAll("[data-shopping-cta]").forEach((el) => {
      ...
      el.addEventListener("click", (ev) => {
        ...
        if (!href || href === "#") {
          ev.preventDefault();
          alert("Todavía no tenemos un enlace seguro para esta oferta.");
        }
      });
    });
  }
  ...
  function renderShoppingSearchResult(data, query) {
    ...
    if (title) {
      title.textContent = data.mode === "live"
        ? `Mejores opciones encontradas para "${query}"`
  ...
  function renderBuscaResults(list, query) {
    ...
    if (!list.length) {
      ...
      if (title) title.textContent = query ? `No se encontró nada para "${query}"` : "Comparativos publicados";
      return;
    }
    if (empty) empty.hidden = true;
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (empty && data.queued) {
            empty.hidden = false;
            empty.querySelector("p").textContent = `Estamos generando el comparativo de "${q}" ahora. Esto toma unos minutos: vuelve y busca de nuevo en un rato.`;
            return;
          }
        }
      } catch {
